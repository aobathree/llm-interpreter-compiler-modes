...
初版 2026年8月1日／改訂版 2026年8月2日（Rust実験からスケール実験・本家への提案までを追記）
...
Rust版は、まだ `bitbank` CLI をサブプロセスとして起動しています（62銘柄で124プロセス）。ならば CLI を経由せず REST API を直接呼べばさらに速いはず——という仮説で、第3段の実装（リポジトリの `rust-direct/`）を作りました。日足＝年間ファイル2本・時足＝直近2日分の計4リクエスト/銘柄を、接続プーリング *[17]* 付きで並列発行します。出力はCLI経由版と1文字単位で同一です。

結果は控えめでした。62銘柄では約1.2倍・ばらつき大幅減となったものの、3〜10銘柄では差が出ません。当初の仮説「プロセス生成とTLSのやり直しが重い」は外れ——`bitbank` CLI 自体が Rust 製の薄いバイナリで、そのコストはネットワーク待ちに埋もれる程度でした。しかも初版はCLI経由版より遅く、原因は (1) HTTPライブラリの接続プールが既定でホストあたり1本しかアイドル接続を保持せず、並列の波ごとにTLSをやり直していた、(2) 時足を必要以上に3日分取得していた、の2つでした。両方を直して逆転しています（詳細は `benchmark/results-direct.md`）。

| 指標（62銘柄） | CLI経由版 | REST API直接版 |
|---|---|---|
| 壁時計時間 | 1.17〜1.74秒 | **1.10〜1.14秒（約1.2倍・安定）** |
| CPU時間合計（プロセスツリー全体） | 1.97秒 | **0.05秒（42倍）** |
| 総プロセス数 | 125 | 1 |
| ピークメモリ（全体） | 30MB | 20MB |

教訓は2つあります。**仮説は計測で潰れる**こと。そして、コンパイルの深化は第2段（Rust化・並列化）で果実の大半を取り終えており、**第3段（呼び出し経路を1層削る）の限界効用は小さい**こと。律速は依然としてネットワークとレート制限にあります。

——と結論しかけたのですが、リソース消費の観点で再計測すると、壁時計時間の裏に大きな差が隠れていました。プロセスツリー全体（親＋124個の子プロセス）をOSのジョブ機能で正確に集計した結果が上表の下段です。CPU時間は**42倍**、プロセス数は125対1。今回の多コア機では、この約2秒分のCPU仕事が16並列の通信待ちの陰に完全に隠れて壁時計時間に現れません。しかし1〜2コアの非力なマシン（格安VPSや小さなコンテナ枠）では、CPU時間がそのまま壁時計時間に跳ね返ります。教訓をもう1つ追加します——**「速くならない」と「軽くならない」は別の話**。どの次元の性能を見るかで、最適化の価値は変わるのです。
...
また、実装した軽量Rust版 periodical-brief は提案のまま眠らせず、著者自身が AWS 上で**1時間ごとに定期実行**し、ダイジェストを Discord に配信する形で日々の実運用に入れています（リポジトリの `aws-hourly-brief/` に設計書と実装を収録。EventBridge → Lambda のサーバーレス構成で、対象銘柄は DynamoDB 経由で動的に変更でき、運用コストは実質ゼロです）。リソース消費42倍差の実験が示したとおり、この「軽さ」こそが最小構成のクラウドで常時運用できる理由になりました。
